...
listFiles = do
    ...
    putStrLn $ "Files count: " ++ show count
...
createMsg = do
  ...
  messageToPrint <- readFile "msg.txt"
  putStrLn $ "Message you wrote: " ++ messageToPrint
...
timeIO :: IO a -> IO Float
timeIO excutable = do
  beforeCpuTime <- getCPUTime
  excutable
  afterCpuTime <- getCPUTime
  return (fromIntegral afterCpuTime - fromIntegral beforeCpuTime)
...
benchmark = do
  ...
  firstTime <- timeIO (return $ primes1 limit)
  let number1 = last (primes1 limit)

  putStrLn $ "Primes1\n\tNumber: " ++ show number1 ++ "\n\tTime: " ++ show (firstTime / 1000000000000.0) ++ " sec"

  secondTime <- timeIO (return $ primes2 limit)
  let number2 = last (primes2 limit)

  putStrLn $ "Primes2\n\tNumber: " ++ show number2 ++ "\n\tTime: " ++ show  (secondTime / 1000000000000.0) ++ " sec"
  
  thirdTime <- timeIO (return $ primes3 limit)
  let number3 = last (primes3 limit)

  putStrLn $ "Primes3\n\tNumber: " ++ show number3 ++ "\n\tTime: " ++ show (thirdTime / 1000000000000.0) ++ " sec"
